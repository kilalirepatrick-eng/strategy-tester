        ...
        <div style={{ padding: "20px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {activeTab === "chart" && result && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                ...
                <input
                  type="range" min={0} max={candles.length - CANDLES_SHOWN} value={visibleStart}
              ...
              <div style={{ display: "flex", gap: 16, fontSize: 10, color: "#4a6070" }}>
                <span style={{ color: "#00d97e" }}>▲ BUY entry</span>
                <span style={{ color: "#ff4757" }}>▼ SELL entry</span>
                <span style={{ color: "#00d4ff" }}>▬ Fast EMA ({cfg.fastEMA})</span>
                <span style={{ color: "#ff6b35" }}>▬ Slow EMA ({cfg.slowEMA})</span>
                <span style={{ color: "#a78bfa" }}>▬ Trend EMA ({cfg.trendEMA})</span>
              </div>
            </div>
          )}
          ...
          {activeTab === "equity" && result && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ background: "#0a0e1a", borderRadius: 8, padding: 16, border: "1px solid #1e2a3a" }}>
                <div style={{ fontSize: 10, color: "#4a6070", marginBottom: 12, letterSpacing: "0.1em", textTransform: "uppercase" }}>Equity Curve — {result.stats.total} trades</div>
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={result.curve}>
                    ...
                    <XAxis dataKey="trade" tick={{ fill: "#4a6070", fontSize: 10, fontFamily: "monospace" }} />
                    <YAxis tick={{ fill: "#4a6070", fontSize: 10, fontFamily: "monospace" }} width={70} />
                    <Tooltip
                      contentStyle={{ background: "#0d1220", border: "1px solid #1e2a3a", borderRadius: 6, fontFamily: "monospace", fontSize: 11 }}
                      formatter={(v) => [`$${v.toLocaleString()}`, "Equity"]}
                    />
                    <ReferenceLine y={10000} stroke="#1e2a3a" strokeDasharray="3 3" />
                    <Area type="monotone" dataKey="equity" stroke="#00d4ff" strokeWidth={2} fill="url(#eqGrad)" dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <div style={{ background: "#0a0e1a", borderRadius: 8, padding: 16, border: "1px solid #1e2a3a" }}>
                <div style={{ fontSize: 10, color: "#4a6070", marginBottom: 12, letterSpacing: "0.1em", textTransform: "uppercase" }}>Drawdown %</div>
                <ResponsiveContainer width="100%" height={120}>
                  <AreaChart data={result.curve}>
                    ...
                    <XAxis dataKey="trade" tick={{ fill: "#4a6070", fontSize: 10 }} />
                    <YAxis tick={{ fill: "#4a6070", fontSize: 10 }} width={40} />
                    <Tooltip contentStyle={{ background: "#0d1220", border: "1px solid #1e2a3a", borderRadius: 6, fontFamily: "monospace", fontSize: 11 }} formatter={(v) => [`${v}%`, "DD"]} />
              ...
              <div style={{ background: "#0a0e1a", borderRadius: 8, padding: 16, border: "1px solid #1e2a3a" }}>
                <div style={{ fontSize: 10, color: "#4a6070", marginBottom: 12, letterSpacing: "0.1em", textTransform: "uppercase" }}>Trade P&L Distribution (pips)</div>
                <ResponsiveContainer width="100%" height={120}>
                  <BarChart data={result.trades.map((t, i) => ({ i, pnl: +t.pnl.toFixed(1) }))}>
                    <XAxis dataKey="i" hide />
                    <YAxis tick={{ fill: "#4a6070", fontSize: 10 }} width={40} />
                    <Tooltip contentStyle={{ background: "#0d1220", border: "1px solid #1e2a3a", borderRadius: 6, fontFamily: "monospace", fontSize: 11 }} formatter={(v) => [`${v} pips`, "P&L"]} />
                    <Bar dataKey="pnl" radius={[2, 2, 0, 0]}>
                      {result.trades.map((t, i) => <Cell key={i} fill={t.result === "WIN" ? "#00d97e" : "#ff4757"} opacity={0.85} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
          ...
          {activeTab === "rsi" && result && (
            <div style={{ background: "#0a0e1a", borderRadius: 8, padding: 16, border: "1px solid #1e2a3a" }}>
              <div style={{ fontSize: 10, color: "#4a6070", marginBottom: 12, letterSpacing: "0.1em", textTransform: "uppercase" }}>RSI ({cfg.rsiPeriod}) — Last 200 candles</div>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={result.rsi.slice(-200).map((v, i) => ({ i, rsi: v })).filter(d => d.rsi !== null)}>
                  <XAxis dataKey="i" hide />
                  <YAxis domain={[0, 100]} tick={{ fill: "#4a6070", fontSize: 10, fontFamily: "monospace" }} width={35} />
                  <Tooltip contentStyle={{ background: "#0d1220", border: "1px solid #1e2a3a", borderRadius: 6, fontFamily: "monospace", fontSize: 11 }} formatter={(v) => [v?.toFixed(1), "RSI"]} />
                  <ReferenceLine y={cfg.rsiOB} stroke="#ff4757" strokeDasharray="4 4" label={{ value: `OB ${cfg.rsiOB}`, fill: "#ff4757", fontSize: 10, fontFamily: "monospace" }} />
                  <ReferenceLine y={cfg.rsiOS} stroke="#00d97e" strokeDasharray="4 4" label={{ value: `OS ${cfg.rsiOS}`, fill: "#00d97e", fontSize: 10, fontFamily: "monospace" }} />
                  <ReferenceLine y={50} stroke="#1e2a3a" />
                  <Line type="monotone" dataKey="rsi" stroke="#ffd700" strokeWidth={1.5} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {activeTab === "trades" && result && (
            <div style={{ background: "#0a0e1a", borderRadius: 8, border: "1px solid #1e2a3a", overflow: "hidden" }}>
              <div style={{ padding: "12px 16px", borderBottom: "1px solid #1e2a3a", fontSize: 10, color: "#4a6070", letterSpacing: "0.1em", textTransform: "uppercase" }}>
                Last {Math.min(20, result.trades.length)} Trades (of {result.trades.length} total)
              </div>
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, fontFamily: "monospace" }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid #1e2a3a" }}>
                      {["#", "TYPE", "ENTRY", "EXIT", "SL", "TP", "PIPS", "RESULT"].map(h => (
                        <th key={h} style={{ padding: "8px 12px", textAlign: "left", color: "#4a6070", fontWeight: 600, fontSize: 10, letterSpacing: "0.1em" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {tradeRows.map((t, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #0d1220", transition: "background 0.1s" }}
                        onMouseEnter={e => e.currentTarget.style.background = "#0d1528"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                      >
                        <td style={{ padding: "9px 12px", color: "#4a6070" }}>{result.trades.length - i}</td>
                        <td style={{ padding: "9px 12px", color: t.type === "BUY" ? "#00d97e" : "#ff4757", fontWeight: 700 }}>{t.type}</td>
                        <td style={{ padding: "9px 12px", color: "#ccd6f6" }}>{t.entry?.toFixed(5)}</td>
                        <td style={{ padding: "9px 12px", color: "#ccd6f6" }}>{t.exit?.toFixed(5)}</td>
                        <td style={{ padding: "9px 12px", color: "#ff475799" }}>{t.sl?.toFixed(5)}</td>
                        <td style={{ padding: "9px 12px", color: "#00d97e99" }}>{t.tp?.toFixed(5)}</td>
                        <td style={{ padding: "9px 12px", color: t.pnl >= 0 ? "#00d97e" : "#ff4757", fontWeight: 600 }}>
                        ...
                        <td style={{ padding: "9px 12px" }}>
                          <span style={{
                            ...
                            borderRadius: 3,
                            fontSize: 10,
                            background: t.result === "WIN" ? "#00d97e22" : "#ff475722",
                            color: t.result === "WIN" ? "#00d97e" : "#ff4757",
                            border: `1px solid ${t.result === "WIN" ? "#00d97e44" : "#ff475744"}`,
                          }}>{t.result}</span>
                        </td>
                      </tr>
                    ))}
              ...
              {/* Summary row */}
              {result && (
                <div style={{ padding: "12px 16px", borderTop: "1px solid #1e2a3a", display: "flex", gap: 20, fontSize: 11, color: "#4a6070" }}>
                  <span>Avg Win: <span style={{ color: "#00d97e" }}>+{result.stats.avgWin} pips</span></span>
                  <span>Avg Loss: <span style={{ color: "#ff4757" }}>-{result.stats.avgLoss} pips</span></span>
                  <span>Expectancy: <span style={{ color: +result.stats.pf >= 1 ? "#00d97e" : "#ff4757" }}>PF {result.stats.pf}</span></span>
                </div>
              )}
            </div>
          )}
